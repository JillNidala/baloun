...
- Change ONLY what the task asks for. Everything else in the input image must be
  reproduced EXACTLY as it is: same colours, same objects, same materials.
- The input image may already contain earlier customisations (a painted wall,
  furniture, clothing on the figure). Those are intentional: KEEP THEM ALL.
...
/** Riassume le modifiche già applicate, per non farle disfare. */
function daPreservare(edits: { kind: string; prompt: string }[]): string {
  if (edits.length === 0) return ''
  const nomi: Record<string, string> = {
    wall: 'the wall',
    decoration: 'an object on the floor',
    outfit: "the figure's clothing",
  }
  const righe = edits
    .slice(-6)
    .map((e) => `- ${nomi[e.kind] ?? e.kind}: "${e.prompt}"`)
    .join('\n')
  return `

ALREADY APPLIED to this room in previous steps. Keep every one of them intact:
${righe}`
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Immagine di partenza. Se il client non la manda (versione vecchia
    // dell'app), la ricaviamo dall'origine della richiesta.
    const origine = req.headers.get('origin') ?? Deno.env.get('APP_URL') ?? ''
    const baseUrl: string =
      (typeof body?.baseUrl === 'string' && body.baseUrl) ||
      (origine ? `${origine.replace(/\/$/, '')}/scene/room.jpg` : '')

    if (!baseUrl) {
      return json({
        error: "Non trovo l'immagine di partenza della stanza.",
        detail:
          "Aggiorna l'app oppure imposta il segreto APP_URL con l'indirizzo del sito.",
      }, 400)
    }

    // Sicurezza: si scarica solo dal sito dell'app o dal proprio Storage,
    // altrimenti la funzione potrebbe essere usata per leggere indirizzi interni.
    const consentiti = [origine, Deno.env.get('APP_URL') ?? '', Deno.env.get('SUPABASE_URL') ?? '']
      .filter(Boolean)
      .map((u) => u.replace(/\/$/, ''))

    if (!baseUrl.startsWith('https://') || !consentiti.some((u) => baseUrl.startsWith(u))) {
      return json({
        error: 'Immagine di partenza non consentita.',
        detail: 'Sono ammessi solo il sito dell\'app e il tuo Storage.',
      }, 400)
    }
    ...
    // Modifiche già applicate: vanno ricordate a Gemini una per una.
    const { data: personalizzazione } = await supabase
      .from('room_customizations')
      .select('edits')
      .eq('room_id', room.id)
      .maybeSingle()

    const storico = (personalizzazione?.edits ?? []) as { kind: string; prompt: string }[]
          ...
          contents: [
            {
              ...
              parts: [
                ...
                { text: ricetta(testo) + daPreservare(storico) },
              ],
            },
          ],
    ...
    await supabase.from('room_customizations').upsert(
      {
        ...
        // lo storico serve alle richieste successive
        edits: [...storico, { kind, prompt: testo }].slice(-10),
