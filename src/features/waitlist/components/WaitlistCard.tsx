...
export function WaitlistCard({ keep }: { keep: Keep }) {
      ...
      <div className="shrink-0 text-right">
        {keep.is_closed ? (
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-balloon">
            {keep.decision === 'kept' ? 'Rivedi' : 'Entra'}
          </span>
        ) : (
  ...
  // Anche dopo aver deciso si deve poter rientrare a rivedere il profilo:
  // prima la scheda si bloccava e non c'era più modo di aprirla.
  const canEnter = open && !keep.is_closed
