...
import { ReactionCards } from '@/features/room/components/ReactionCards'
...
export function RoomsPage() {
  ...
  // Quando un pannello copre la pagina, il trascinamento va disattivato.
  const [bloccaSwipe, setBloccaSwipe] = useState(false)
    ...
    <div className="safe-top py-4">
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            {tab === 'mine' ? <MyRoomSection onOverlay={setBloccaSwipe} /> : <OthersSection />}
          </motion.div>
        </AnimatePresence>
      </motion.div>
    </div>
...
function MyRoomSection({ onOverlay }: { onOverlay: (v: boolean) => void }) {
  ...
  // avvisa la pagina quando un pannello è aperto
  useEffect(() => {
    onOverlay(Boolean(balloon) || Boolean(matched))
  }, [balloon, matched, onOverlay])
  ...
  return (
    <>
      ...
      <ReactionCards reactions={reactions ?? []} />
