...
-- Storico delle modifiche chieste: serve a dire a Gemini cosa NON deve
-- disfare quando gliene chiedi una nuova.
alter table public.room_customizations
...
-- ------------------------------------------------------------
-- 7d. INFORMAZIONI PERSONALI E IMPOSTAZIONI
--
-- Nome ed età non si modificano (come su Hinge): restano su profiles.
-- Città e professione sì.
--
-- Genere, orientamento, religione e opinioni politiche sono dati
-- PARTICOLARI: stanno in una tabella separata che legge solo chi la
-- possiede. Escono verso gli altri solo tramite le funzioni, e solo se
-- l'utente ha scelto di mostrarli.
-- ------------------------------------------------------------
...
do $$ begin
  alter table public.profiles
    ...
    check (profession is null or char_length(profession) <= 60);
exception when duplicate_object then null; end $$;

-- --- cataloghi delle opzioni ---
create table if not exists public.gender_options (
  slug text primary key, label text not null, sort int not null default 0
);
insert into public.gender_options (slug, label, sort) values
  ('donna',            'Donna',                    10),
  ('uomo',             'Uomo',                     20),
  ('donna_trans',      'Donna transgender',        30),
  ('uomo_trans',       'Uomo transgender',         40),
  ('non_binaria',      'Persona non binaria',      50),
  ('genderfluid',      'Genderfluid',              60),
  ('agender',          'Agender',                  70),
  ('intersex',         'Intersex',                 80),
  ('altro',            'Altro',                    90),
  ('non_dico',         'Preferisco non dirlo',    100)
on conflict (slug) do update set label = excluded.label, sort = excluded.sort;

create table if not exists public.religion_options (
  slug text primary key, label text not null, sort int not null default 0
);
insert into public.religion_options (slug, label, sort) values
  ('agnostica',   'Agnostica',              10),
  ('atea',        'Atea',                   20),
  ('buddhista',   'Buddhista',              30),
  ('cattolica',   'Cattolica',              40),
  ('cristiana',   'Cristiana',              50),
  ('ebraica',     'Ebraica',                60),
  ('induista',    'Induista',               70),
  ('musulmana',   'Musulmana',              80),
  ('spirituale',  'Spirituale',             90),
  ('altro',       'Altro',                 100),
  ('non_dico',    'Preferisco non dirlo',  110)
on conflict (slug) do update set label = excluded.label, sort = excluded.sort;

create table if not exists public.politics_options (
  slug text primary key, label text not null, sort int not null default 0
);
insert into public.politics_options (slug, label, sort) values
  ('sinistra',    'Sinistra',               10),
  ('centro_sx',   'Centrosinistra',         20),
  ('centro',      'Centro',                 30),
  ('centro_dx',   'Centrodestra',           40),
  ('destra',      'Destra',                 50),
  ('apolitica',   'Apolitica',              60),
  ('altro',       'Altro',                  70),
  ('non_dico',    'Preferisco non dirlo',   80)
on conflict (slug) do update set label = excluded.label, sort = excluded.sort;

-- --- dati riservati + impostazioni ---
create table if not exists public.profile_settings (
  profile_id uuid primary key references public.profiles (id) on delete cascade,

  -- dati particolari
  gender     text references public.gender_options (slug),
  religion   text references public.religion_options (slug),
  politics   text references public.politics_options (slug),

  -- cosa mostrare agli altri: di serie NIENTE
  show_gender   boolean not null default false,
  show_religion boolean not null default false,
  show_politics boolean not null default false,
  ...
  constraint profile_settings_phone_len  check (phone is null or char_length(phone) between 6 and 24),
  constraint profile_settings_plan_valid check (plan in ('free', 'plus', 'premium')),
  constraint profile_settings_radius     check (search_radius_km between 5 and 500)
);

-- "mi piacciono...": più opzioni insieme
create table if not exists public.profile_interested_in (
  profile_id  uuid not null references public.profiles (id) on delete cascade,
  gender_slug text not null references public.gender_options (slug) on delete cascade,
  primary key (profile_id, gender_slug)
);

alter table public.gender_options        enable row level security;
alter table public.religion_options      enable row level security;
alter table public.politics_options      enable row level security;
alter table public.profile_settings      enable row level security;
alter table public.profile_interested_in enable row level security;

-- i cataloghi li leggono tutti
drop policy if exists gender_options_select on public.gender_options;
create policy gender_options_select on public.gender_options
  for select to authenticated using (true);

drop policy if exists religion_options_select on public.religion_options;
create policy religion_options_select on public.religion_options
  for select to authenticated using (true);

drop policy if exists politics_options_select on public.politics_options;
create policy politics_options_select on public.politics_options
  for select to authenticated using (true);

-- i dati riservati li vede SOLO chi li possiede
drop policy if exists profile_settings_own on public.profile_settings;
create policy profile_settings_own on public.profile_settings
  for all to authenticated
  using (profile_id = auth.uid()) with check (profile_id = auth.uid());

drop policy if exists interested_in_own on public.profile_interested_in;
create policy interested_in_own on public.profile_interested_in
  for all to authenticated
  using (profile_id = auth.uid()) with check (profile_id = auth.uid());

-- alla registrazione nasce anche la riga delle impostazioni
create or replace function public.crea_impostazioni()
returns trigger language plpgsql security definer set search_path = public as $$
begin
...
drop trigger if exists profiles_crea_impostazioni on public.profiles;
create trigger profiles_crea_impostazioni
  after insert on public.profiles
  for each row execute function public.crea_impostazioni();

-- e per chi è già registrato
...
language sql stable security definer set search_path = public as $$
  with io as (
    select
      p.id,
      p.city,
      ps.gender          as mio_genere,
      ps.search_city,
      -- i generi che mi interessano; vuoto = nessuna preferenza
      coalesce(array_agg(ii.gender_slug) filter (where ii.gender_slug is not null), '{}') as cerco
    from public.profiles p
    left join public.profile_settings ps      on ps.profile_id = p.id
    left join public.profile_interested_in ii on ii.profile_id = p.id
    where p.id = auth.uid()
    group by p.id, p.city, ps.gender, ps.search_city
  )
  select
    p.id,
    r.id,
    ...
    -- punteggio: interessi in comune, stessa città, poi il caso
    (
      (select count(*)::int * 2
         from public.profile_tags a
         join public.profile_tags b
           on b.tag_slug = a.tag_slug and b.profile_id = io.id
        where a.profile_id = p.id)
      + case when p.city is not null
                  and p.city = coalesce(io.search_city, io.city) then 3 else 0 end
    ) as affinita
  from public.rooms r
  join public.profiles p on p.id = r.main_id
  join public.profile_settings ps_altro on ps_altro.profile_id = p.id
  cross join io
  where r.status = 'open'
    ...
    -- ALGORITMO, primo criterio: deve piacermi il suo genere.
    -- Se non ho indicato preferenze, non filtro.
    and (
      cardinality(io.cerco) = 0
      or ps_altro.gender is null
      or ps_altro.gender = any (io.cerco)
    )

    -- e io devo rientrare in ciò che cerca lei/lui: l'interesse è reciproco
    and (
      io.mio_genere is null
      or not exists (select 1 from public.profile_interested_in x where x.profile_id = p.id)
      or exists (
        select 1 from public.profile_interested_in x
         where x.profile_id = p.id and x.gender_slug = io.mio_genere
      )
    )

    and not exists (
      select 1 from public.room_waitlist w
       where w.room_id = r.id and w.balloon_id = io.id
    )
    and (select count(*) from public.room_waitlist w2
          where w2.room_id = r.id and w2.decision <> 'popped') < r.max_balloons
  order by affinita desc, r.created_at desc
...
language sql stable security definer set search_path = public as $$
  select
    ...
    p.profession,
    -- solo ciò che il proprietario ha scelto di mostrare
    (select jsonb_strip_nulls(jsonb_build_object(
       'gender',   case when ps.show_gender   then g.label end,
       'religion', case when ps.show_religion then rl.label end,
       'politics', case when ps.show_politics then pol.label end
     ))
     from public.profile_settings ps
     left join public.gender_options   g   on g.slug   = ps.gender
     left join public.religion_options rl  on rl.slug  = ps.religion
     left join public.politics_options pol on pol.slug = ps.politics
