...
-- ------------------------------------------------------------
-- 1c. Genere e preferenze per chi non li ha: senza questi dati
--     l'algoritmo non saprebbe a chi mostrarli.
-- ------------------------------------------------------------
insert into public.profile_settings (profile_id, gender, show_gender)
select p.id,
       (array['donna','uomo','non_binaria'])[1 + (abs(hashtext(p.id::text)) % 3)],
       true
from public.profiles p
where p.onboarded
on conflict (profile_id) do update
  set gender = coalesce(public.profile_settings.gender, excluded.gender);

-- ciascuno "cerca" due generi, così gli incroci funzionano in entrambi i sensi
insert into public.profile_interested_in (profile_id, gender_slug)
select p.id, g
from public.profiles p
cross join lateral (
  select unnest(
    case (abs(hashtext(p.id::text || 'x')) % 3)
      when 0 then array['donna','non_binaria']
      when 1 then array['uomo','non_binaria']
      else        array['donna','uomo']
    end
  ) as g
) sub
where p.onboarded
  and not exists (
    select 1 from public.profile_interested_in ii where ii.profile_id = p.id
  )
on conflict do nothing;
