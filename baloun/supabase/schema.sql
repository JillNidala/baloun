...
language sql stable security definer set search_path = public as $$
  select
    ...
    c.outfit_path,
    coalesce(
      (select jsonb_agg(jsonb_build_object('slot', d.slot, 'path', d.image_path) order by d.slot)
         from public.room_decorations d where d.room_id = r.id),
      '[]'::jsonb
    )
  from public.rooms r
...
language sql stable security definer set search_path = public as $$
  select rr.id, rr.emoji, rr.sender_id, rr.created_at
...
-- ------------------------------------------------------------
-- 15. PERSONALIZZAZIONE DELLA STANZA
...
create table if not exists public.room_customizations (
  ...
  main_id     uuid not null references public.profiles (id) on delete cascade,
  wall_path   text,          -- immagine applicata al muro (ritagliata dalla maschera)
  outfit_path text,          -- omino vestito, sostituisce quello di serie
  updated_at  timestamptz not null default now()
);
