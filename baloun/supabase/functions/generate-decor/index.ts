...
// I nomi dei modelli cambiano spesso. Invece di fissarli, chiediamo alla
// chiave quali modelli ha davvero a disposizione e scegliamo un modello
// capace di generare immagini. Il segreto GEMINI_MODEL, se presente, vince.
const API = 'https://generativelanguage.googleapis.com/v1beta'

let modelloScelto: string | null = null

async function elencaModelli(apiKey: string): Promise<
...
/** Sceglie un modello che sappia restituire immagini. */
async function trovaModello(apiKey: string): Promise<string | null> {
  ...
  const candidati = modelli
    ...
    // i più recenti hanno numeri più alti nel nome
    .sort((a, b) => b.name.localeCompare(a.name, 'en', { numeric: true }))

  modelloScelto = candidati[0]?.name ?? null
  ...
  try {
    ...
    if (body?.ping) {
      ...
      return json({
        ...
        modelli_per_immagini: perImmagini,
        scelto: await trovaModello(apiKey),
      })
    }
    ...
    const model = await trovaModello(apiKey)
    if (!model) {
      const disponibili = (await elencaModelli(apiKey))
        .filter((m) => m.methods.includes('generateContent'))
        .map((m) => m.name)
      return json({
        error: 'La tua chiave non ha modelli capaci di generare immagini.',
        detail: 'Modelli disponibili: ' + (disponibili.join(', ') || 'nessuno'),
      }, 400)
    }
    ...
    if (!res.ok) {
      ...
      modelloScelto = null // al prossimo tentativo si riparte dalla scoperta
      return json(
        { error: `Gemini ha rifiutato la richiesta (${model}).`, detail: detail.slice(0, 400) },
        502,
      )
    }
