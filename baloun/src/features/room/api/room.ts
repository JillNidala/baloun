...
type SceneRow = {
  ...
  is_open: boolean
  is_closed: boolean
  wall_path: string | null
  outfit_path: string | null
  decorations: { slot: number; path: string }[]
  balloons: {
    balloon_id: string
    name: string | null
    small: string | null
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        decor: {
          wallUrl: decorUrl(row.wall_path),
          outfitUrl: decorUrl(row.outfit_path),
          decorations: (row.decorations ?? [])
            .map((d) => ({ slot: d.slot, url: decorUrl(d.path) }))
            .filter((d): d is { slot: number; url: string } => Boolean(d.url)),
        },
      }
    },
  })
