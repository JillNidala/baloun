    ...
    queryFn: async () => {
      ...
      const balloons: SceneBalloon[] = await Promise.all(
        (row.balloons ?? []).map(async (b) => ({
          ...
          name: b.name,
          // il Main vede la foto nitida, gli altri quella piccola (sfocata dall'app)
          photoUrl: row.is_mine
      ...
      return { ...row, sceneBalloons: balloons }
    },
  })
}
...
export function useRoomReactions(roomId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error
      return (data ?? []) as { id: string; emoji: string; created_at: string }[]
    },
  })
}
