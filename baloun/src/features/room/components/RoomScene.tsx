...
type Props = {
  ...
  /** personalizzazione: muro, abito dell'omino, arredi sul pavimento */
  decor?: {
    wallUrl?: string
    outfitUrl?: string
    decorations?: { slot: number; url: string }[]
  }
}

/**
 * Le tre posizioni per gli arredi, al centro del pavimento.
 * Verificate sul render: sono tutte su legno e lontane dall'omino,
 * così non coprono mai né lui né i palloncini.
 */
const DECOR_SLOTS = [
  { x: 540, y: 620, w: 190 },
  { x: 690, y: 672, w: 210 },
...
/** Riquadro dell'omino, per sostituirlo quando cambia vestito. */
const FIGURE = { x: 292, y: 452, w: 120, h: 190 } as const
...
export function RoomScene({ balloons, onSelect, poppingId, onPopEnd, reactions, decor }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Muro personalizzato: l'immagine è ritagliata dalla maschera della
          parete, quindi non può sbordare su tenda o pavimento. */}
      {decor?.wallUrl && (
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: `url(${decor.wallUrl})`,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            WebkitMaskImage: 'url(/scene/wall-mask.png)',
            maskImage: 'url(/scene/wall-mask.png)',
      ...
      {/* Arredi sul pavimento, al centro */}
      {decor?.decorations?.map((d) => {
        const slotDef = DECOR_SLOTS[d.slot] ?? DECOR_SLOTS[0]
        return (
          <img
            key={d.slot}
            src={d.url}
            ...
            style={{
              left: `${pct(slotDef.x, IMG.w)}%`,
              top: `${pct(slotDef.y, IMG.h)}%`,
              width: `${pct(slotDef.w, IMG.w)}%`,
            }}
          />
        )
      })}

      {/* Omino vestito: copre quello di serie */}
      {decor?.outfitUrl && (
        <img
          src={decor.outfitUrl}
          alt=""
          ...
          className="absolute"
          style={{
            left: `${pct(FIGURE.x, IMG.w)}%`,
            top: `${pct(FIGURE.y, IMG.h)}%`,
            width: `${pct(FIGURE.w, IMG.w)}%`,
          }}
        />
      )}
