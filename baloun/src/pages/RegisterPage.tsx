...
export function RegisterPage() {
  ...
  const onSubmit = (data: Form) => {
    // STEP 3: supabase.auth.signUp + creazione riga profiles via trigger.
    signIn(data.email)
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            {...register('password')}
          />
          {errors.password && <span className="font-mono text-[12px] text-balloon">{errors.password.message}</span>}
        </label>
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            {...register('confirm')}
          />
          {errors.confirm && <span className="font-mono text-[12px] text-balloon">{errors.confirm.message}</span>}
        </label>
