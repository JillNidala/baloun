...
import { BalloonSheet } from '@/features/room/components/BalloonSheet'
    ...
    <>
      ...
      <AnimatePresence>
        {balloon && (
          <BalloonSheet
            balloon={balloon}
            popping={pop.isPending}
            onClose={() => setSelected(null)}
            onPop={() => {
              // prima l'animazione dello scoppio, poi la cancellazione
              setSelected(null)
              setPopping(balloon.balloon_id)
...
function OthersSection() {
  ...
  return (
    <div className="flex flex-col gap-8">
      {keeps?.map((keep) => <OtherRoom key={keep.room_id} roomId={keep.room_id} />)}
    </div>
  )
}

function OtherRoom({ roomId }: { roomId: string }) {
  const navigate = useNavigate()
  const { data: scene } = useRoomScene(roomId)
  const { label } = useCountdown(scene?.opens_at)
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex items-center justify-between border-t border-hairline px-4 py-3">
          <div className="min-w-0">
            <p className="truncate font-display text-[19px] font-bold leading-none">
              {scene.main_name}
            </p>
            <p className="caption mt-1.5">{scene.sceneBalloons.length} palloncini</p>
          </div>

          {scene.is_closed ? (
            <span className="caption">Conclusa</span>
          ) : scene.is_open ? (
            <Pressable
              onClick={() => navigate(PATHS.room(roomId))}
              className="rounded-full bg-ink px-4 py-2 font-mono text-[11px] uppercase tracking-[0.12em] text-cream"
            >
              Entra
            </Pressable>
          ) : (
            <span className="font-mono text-[16px] tabular-nums">{label}</span>
          )}
        </div>
      </Card>

      <div className="mt-4">
        <EmojiBar roomId={roomId} />
      </div>
    </div>
  )
}
