...
/** Carica una delle 6 foto del profilo, in entrambe le risoluzioni. */
export async function uploadPhoto(
  ...
  const smallPath = `${userId}/small-${position}.jpg`
  const fullPath = `${userId}/photo-${position}.jpg`
...
/** Rimuove i file di una foto eliminata. */
export async function deletePhotoFiles(fullPath: string, smallPath: string): Promise<void> {
  await Promise.all([
    supabase.storage.from('avatars').remove([smallPath]),
    supabase.storage.from('avatars-full').remove([fullPath]),
  ])
}
